...
async function handleProjectCommand(action, params, context) {
  switch (action) {
    case 'create': {
      ...
      // TODO: Apply SOP template to auto-generate milestone tasks
      ...
      if (project_name) {
        result = await query(
          `SELECT p.*,
           ...
           FROM projects p WHERE p.name ILIKE $1 AND p.status = 'active'`,
          [`%${project_name}%`]
        );
      } else {
        result = await query(
          `SELECT p.*,
            (SELECT COUNT(*) FROM tasks WHERE project_id = p.id) as total_tasks,
            (SELECT COUNT(*) FROM tasks WHERE project_id = p.id AND status = 'done') as done_tasks
           FROM projects p WHERE p.space_id = $1 AND p.status = 'active'
           ORDER BY p.created_at DESC LIMIT 5`,
          [context.spaceId]
        );
      }

      if (result.rows.length === 0) return '找不到進行中的專案。';

      return result.rows.map((p) => {
        const progress = p.total_tasks > 0
          ? `${p.done_tasks}/${p.total_tasks} 完成`
          : '尚無任務';
